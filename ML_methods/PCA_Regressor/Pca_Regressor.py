...
def process_station(station_name, path_to_directory, current_directory, parent_directory):
    ...
    for i in range(NUM_GROUPS):
        ...
        group_df.to_csv(os.path.join(parent_directory, 'ML_methods',
                                 'Reports_Methods', 'Reports_PcaRegressor',
    ...
    for i, group_data in enumerate(group_list):
        ...
        clf = RegressionResidual(regressor=LinearRegression(), target=POWER_ID)
        df_timestamps = pd.DataFrame({'timestamp': time_})
        output_dir = os.path.join(parent_directory, 'ML_methods', 'Reports_Methods', 'Reports_PcaRegressor', DIR_EXP)
        try:
            y_train_pred = clf.fit_transform(group_data)
            target_value, scalers_loss = scaler_loss(y_train_pred.values, 'cdf')
            ...
            df_target_final['timestamp'] = time_
            df_target_final['target_value'] = 0
            df_target_final.to_csv(os.path.join(output_dir, 'csv_predict', f'predict_{i}.csv'), index=False)

            df_loss_final = pd.DataFrame()
            df_loss_final['timestamp'] = time_
            df_loss_final[group_data.columns[0]] = 0
            df_loss_final.to_csv(os.path.join(output_dir, 'csv_loss', f'loss_{i}.csv'), index=False)
            print(f'При обработке {station}')
