...
config_path = os.path.join(path_to_directory, 'config', f'{configs_train[1]}.yml')
...
if ROLLING_MEAN:
    ...
    df.dropna(inplace=True)  # Remove NaN values introduced by rolling mean
...
for i in range(NUM_GROUPS):
    ...
    scaled_group = pd.DataFrame(scaler.fit_transform(group_df), columns=group_df.columns)
...
for i, group_data in enumerate(group_list):
    ...
    average_errors_per_feature = accumulated_errors_per_feature / counts[:, np.newaxis]  # Размерность: (n_samples, n_features)
